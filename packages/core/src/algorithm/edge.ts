...
export const distanceBetweenPoints = (point1: Point, point2: Point, gridSize: number = 1): number => {
  const dx = (point1.x - point2.x) / gridSize;
  const dy = (point1.y - point2.y) / gridSize;
  return Math.sqrt(dx ** 2 + dy ** 2);
};

const closestPointOnLineSegment = (point: Point, lineSegment: LineSegment, gridSize: number): Point => {
  ...
  const dotProduct = ((point.x - start.x) * dx + (point.y - start.y) * dy) / segmentLength;
...
export const closestPointOnPolyline = (point: Point, polyline: LineSegment[], gridSize: number): Point => {
 ...
 */
export const isPointOnLineSegment = (point: Point, start: Point, end: Point): boolean => {
  const { x, y } = point;
  ...
  const distanceStartToPoint = Math.sqrt((x - start.x) ** 2 + (y - start.y) ** 2);
  const distanceEndToPoint = Math.sqrt((x - end.x) ** 2 + (y - end.y) ** 2);

  // Calculate the length of the line segment
  const lineSegmentLength = Math.sqrt((end.x - start.x) ** 2 + (end.y - start.y) ** 2);
  ...
  const epsilon = 1e-6;
  return Math.abs(distanceStartToPoint + distanceEndToPoint - lineSegmentLength) < epsilon;
};
