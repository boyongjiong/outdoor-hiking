...
export class SnaplineModel {
  ...
  // 计算节点上下边框与其它节点的对齐信息
  readonly getHorizontalSnapline = (draggingNode: NodeData, nodes: BaseNodeModel[]): ISnaplineInfo => {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (node.id !== draggingNode.id) {
        ...
        if (nodeData.minY === draggingNodeData.minY
        ...
        // 如果节点的最大和最小 Y 轴坐标与节点的最大最小 Y 轴坐标相等，展示水平线
        if (nodeData.minY === draggingNodeData.maxY
  ...
  readonly getVerticalSnapline = (draggingNode: NodeData, nodes: BaseNodeModel[]): ISnaplineInfo => {
    const draggingNodeData = this.getDraggingNodeData(draggingNode);
    let isShowVertical = false;
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (node.id !== draggingNode.id) {
        const nodeData = getNodeBBox(node);
        // 如果节点的最大和最小 Y 轴坐标与节点的最大最小 Y 轴坐标相等，展示水平线
        if (nodeData.minX === draggingNodeData.minX
        ...
        // 如果节点的最大和最小 Y 轴坐标与节点的最大最小 Y 轴坐标相等，展示水平线
        if (nodeData.minX === draggingNodeData.maxX
  ...
  getSnaplinePosition(draggingNode: NodeData, nodes: BaseNodeModel[]): ISnaplineInfo {
    ...
    if (!isShowHorizontal) {
      ...
      const { isShowHorizontal, position: { y } } = hSnapline;
    ...
    if (!isShowVertical) {
      ...
      const { isShowVertical, position: { x } } = vSnapline;
