...
export class BaseEdgeModel implements IBaseEdgeModel {
  ...
  //Anchors 相关
  getAnchorPosition(sourceNode: BaseNodeModel, targetPosition: LogicFlow.Position) :LogicFlow.Point {
  ...
  getBeginAnchor(sourceNode: BaseNodeModel, targetNode: BaseNodeModel): LogicFlow.Point {
  ...
  getEndAnchor(targetNode: BaseNodeModel): LogicFlow.Point {
    return this.getAnchorPosition(targetNode, this.startPoint as LogicFlow.Position);
  }
  @action setupAnchors() {
    // 先找离目标节点最近的开始节点的锚点
  ...
  @action updateAfterAdjustStartAndEnd({ startPoint, endPoint }: {
    startPoint: LogicFlow.Point;
    endPoint: LogicFlow.Point;
  }) {
  ...
  @action setElementState(state: ElementState, additionStateData?: Model.AdditionStateDataType) {
    this.state = state;
